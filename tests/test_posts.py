...
def test_unauthroized_user_create_posts(client, test_user, test_posts):
...
def test_update_post_non_exit(authorized_client, test_user, test_posts):
    data = {
        "title":"updated title",
        "content": "updated content",
        "id": test_posts[3].id
    }
    res = authorized_client.put(f"/posts/579843", json=data)
    assert res.status_code == 404 
